...
- (NSString *) genKeyForTrackerData:(NSString *)trackerName
    ...
    dateFormat.locale = [[[NSLocale alloc] initWithLocaleIdentifier:@"en_US_POSIX"] autorelease];
    NSString *currentDateStr = [dateFormat stringFromDate:[NSDate date]];
    
    NSString *keyStr = [NSString stringWithFormat:@"%@%@%@%d",trackerName,fieldsStr,currentDateStr,LOG_FORMAT_VERSION];
    
    //    ZDLogDebug(@"genKeyForTrackerData key str:%@",keyStr);
    [dateFormat release];
...
- (void) _cacheLogData:(NSNotification *)notification{
    ...
    @synchronized(self){
        ...
        //      ZDLogDebug(@"%@", [NSString urlParametersStringFromDictionary:[[notification userInfo] objectForKey:@"params"]]);
        ...
        if ([paramsStr length] > [[NLogConfig get:@"sendMaxLength"] integerValue] * 1024) {
            NPrintLog(@"Log's too long with length:%ld",(long)[paramsStr length]);
            return;
        }
        ...
        if ( lastItem == nil || [lastItem length] + [paramsStr length] > [[NLogConfig get:@"sendMaxLength"] integerValue] * 1024 ) {
            ...
            //            ZDLogDebug(@"before encrypt length:%d", [paramsStr length]);
            
#ifndef NLOG_DEBUG_MODE
            paramsStr = [NStringExtension encrypt:paramsStr];
#endif
            
            //            ZDLogDebug(@"after encrypt length:%d", [paramsStr length]);
            
            //            ZDLogDebug(@"after unencrypt:%@", [paramsStr unencrypt]);
