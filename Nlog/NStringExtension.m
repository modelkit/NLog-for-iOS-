...
+ (NSString *)encrypt:(NSString *)sourceStr
{
    ...
    @catch(NSException* ex){        
        ZDLogDebug(@"At NStringExtension encrypt, reason: %@", [ex reason]);
    }
...
+ (NSString *)unencrypt:(NSString *)sourceStr
{
    ...
    unsigned char * targetBytes = malloc(sizeof(unsigned char)*targetLength);
    memset(targetBytes, 0, targetLength);
    int i = 0;
    ...
    unsigned char * resultBuffer =  malloc(targetLength+1);
