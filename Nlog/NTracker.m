...
- (id)initWithId:(NSString *)trackerId{
    ...
    if (self) {
        ...
        // 应用版本
        NSString* av = [[[NSBundle mainBundle] infoDictionary] objectForKey:@"CFBundleShortVersionString"];
...
- (void) send:(NSString *)hitType params:(NSDictionary *)params immediately:(BOOL)now{
    if (running == NO || ![self isHitRate]) {
        ZDLogDebug(@"MAC Address not hit the rate, so no data sent.");
        return;
    }
...
- (BOOL) isHitRate{
    ...
    /*
     ZDLogDebug(@"Mac Address:%@", macAddr);
     ZDLogDebug(@"Hit tail:%u", hitTail);
     ZDLogDebug(@"Rate:%f", rate);
     ZDLogDebug(@"Max:%f", max);
     */
