...
//#define OpenUDIDLog(fmt, ...) ZDLogDebug((@"%s [Line %d] " fmt), __PRETTY_FUNCTION__, __LINE__, ##__VA_ARGS__);
//#define OpenUDIDLog(fmt, ...) ZDLogDebug((@"[Line %d] " fmt), __LINE__, ##__VA_ARGS__);
...
+ (NSString*) valueWithError:(NSError **)error {
    ...
    if(appUID == nil)
    {
      // generate a new uuid and store it in user defaults
        ...
        CFRelease(uuid);
#warning mrc--->arc   [appID autorelease] (old)
