...
- (void) downloadPolicyFile:(id)arg{
    @autoreleasepool {
        ...
        @try {
            ...
            NPrintLog(@"Policy file response code: %ld", (long)[urlResponse statusCode]);
            
            if ([urlResponse statusCode] >= 200 && [urlResponse statusCode] < 300) {
                //ZDLogDebug(@"Response: %@", result);
                
                //新的API可以从NSData直接转至字典
                NSError *err;
                NSMutableDictionary *jsonResult = [NSJSONSerialization JSONObjectWithData:responseData
                                                                    options:NSJSONReadingMutableContainers
                                                                      error:&err];
                
                //NSMutableDictionary *jsonResult = [result mutableObjectFromJSONString];老代码
